...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        # New args, compatible with old config
        block_size = getattr(generation_config, "block_size", 32)
        print_all_token_records = getattr(generation_config, "print_all_token_records", True)
        ...
        print(
            f"======== Dream block decoding: alg={alg}, temperature={temperature}, "
        ...
        for block_idx in range(num_blocks):
            ...
            # Each block has its own denoising schedule
            ...
            for local_step_idx in range(steps_per_block):
                ...
                # Whole sequence already finished
                ...
                t = timesteps[local_step_idx]
                s = timesteps[local_step_idx + 1]

                # Number of current-block remaining masks per batch
                num_mask_token = current_block_mask_index.sum(dim=1)

                if local_step_idx < steps_per_block - 1:
                    number_transfer_tokens = (num_mask_token.float() * (1 - s / t)).long()
                    number_transfer_tokens = torch.clamp(number_transfer_tokens, min=1)
                    number_transfer_tokens = torch.minimum(number_transfer_tokens, num_mask_token)
                else:
                    number_transfer_tokens = num_mask_token

                max_transfer = int(number_transfer_tokens.max().item())
                ...
                # For top-k selection only inside current block
                confidence_for_select = torch.full_like(confidence_full, -torch.inf)
                confidence_for_select[current_block_mask_index] = confidence_full[current_block_mask_index]
                ...
                if max_transfer > 0:
                    if alg_temp is None or alg_temp == 0:
                        _, transfer_index = torch.topk(
                            confidence_for_select,
                            k=max_transfer,
                            dim=1,
                        )
                    else:
                        sampling_conf = confidence_for_select / alg_temp
                        sampling_prob = F.softmax(sampling_conf, dim=-1)
                        transfer_index = torch.multinomial(
                            sampling_prob,
                            num_samples=max_transfer,
                        )

                    # Different batch items may need different number of tokens
                    for batch_idx in range(x.size(0)):
                        k = int(number_transfer_tokens[batch_idx].item())
                        if k <= 0:
                            continue

                        selected_pos = transfer_index[batch_idx, :k]
                        selected_mask[batch_idx, selected_pos] = True
                ...
                if log:
                    ...
                    print(
                        ...
                        f"selected={int(selected_mask.sum().item())}, "
