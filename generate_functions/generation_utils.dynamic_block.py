...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        decoding_method = (
            "calibrated_token_threshold_adaptive_parallel"
            if len(token_threshold_map) > 0
            else "fixed_confidence_adaptive_parallel"
        )
        ...
        for block_idx in range(num_blocks):
            ...
            for local_step_idx in range(steps_per_block):
                ...
                # =========================
                # Calibrated token-level threshold selection
                # =========================
                ...
                for batch_idx in range(x.size(0)):
                    ...
                    if is_last_step_in_block:
                        ...
                        for pos in candidate_positions:
                            ...
                            if conf >= tau:
                                selected_positions_list.append(pos)

                        if len(selected_positions_list) > 0:
                            selected_positions = torch.stack(selected_positions_list).to(
                                device=x.device,
                                dtype=torch.long,
                            )
                        else:
                            # Fallback: if no token exceeds its threshold,
                            # decode the highest-confidence one to avoid deadlock.
                ...
                for batch_idx in range(x.size(0)):
                    ...
                    for pos in candidate_positions:
                        ...
                        token_id = int(x0_full[batch_idx, pos_int].item())
                        conf = float(confidence_full[batch_idx, pos_int].item())
                        tau = float(token_threshold_map.get(token_id, fixed_threshold))
                        ...
                        all_token_records.append({
                            ...
                            "token_id": token_id,
                            "threshold": tau,
                            "threshold_source": "calibrated" if token_id in token_threshold_map else "default",
